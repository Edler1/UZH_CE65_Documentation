...
efficiency = data.Get("AnalysisEfficiency/"+apts+"/pixelEfficiencyMap_trackPos")
...
for j in range(0,NBINS):
    for i in range(0,NBINS):
        counter +=1
        eff_merged.append(100*efficiency_merged.GetEfficiency(i+(NBINS+2)*j+NBINS+3))
        errup_merged.append(100*efficiency_merged.GetEfficiencyErrorUp(i+(NBINS+2)*j+NBINS+3))
        errlow_merged.append(100*efficiency_merged.GetEfficiencyErrorLow(i+(NBINS+2)*j+NBINS+3))
...
for j in range(0,NBINS):
    for i in range(0,NBINS):
        eff.append(100*efficiency.GetEfficiency(i+(NBINS+2)*j+NBINS+3))
        errup.append(100*efficiency.GetEfficiencyErrorUp(i+(NBINS+2)*j+NBINS+3))
        errlow.append(100*efficiency.GetEfficiencyErrorLow(i+(NBINS+2)*j+NBINS+3))
...
tot_eff = data.Get("AnalysisEfficiency/"+apts+"/eTotalEfficiency").GetEfficiency(1)*100
...
plt.sca(ax[0])
im = plt.imshow(eff,extent=[-pitch/2.,pitch/2.,-pitch/2.,pitch/2.],vmin=min_eff,vmax=100,origin='lower')
...
cb.set_label("Detection efficiency (%)", fontsize=font_size_plots)
...
plt.ylabel("Detection efficiency (%)", fontsize=font_size_plots)
secax = ax[1].secondary_xaxis('top')
secax.set_xticks([x[0][0],x[1][0],x[2][0],x[2][-1]])
for i,t in enumerate(secax.set_xticklabels(['A','B','C','A'])):
    t.set_color(c[i%3])
plt.axhline(tot_eff,linestyle='dotted',color='grey', linewidth=3.5)
plt.sca(ax[1])
plt.text(0.08, tot_eff - 0.5, "Total efficiency", fontsize=font_size_tot_eff, color='grey')
# plt.text(14.5,tot_eff,f"Total efficiency",ha='center', va='bottom',color='grey', fontsize=font_size_plots)
...
if pitch == 22.5:
    ...
    ax[1].plot([x_start + 0.5*2.25, x_start + 0.5*2.25 + 2], [y_start + 0.4 + 0.5, y_start + 0.4 + 0.5], color=c[0], linestyle='-', linewidth=1, zorder=10)
    ax[1].plot([x_start + 1.5*2.25, x_start + 1.5*2.25 + 2], [y_start + 0.4 + 0.5, y_start + 0.4 + 0.5], color=c[1], linestyle='-', linewidth=1, zorder=10)
    ax[1].plot([x_start + 2.5*2.25, x_start + 2.5*2.25 + 2], [y_start + 0.4 + 0.5, y_start + 0.4 + 0.5], color=c[2], linestyle='-', linewidth=1, zorder=10)
...
if pitch == 10:
    ...
    plt.annotate(xy=(x_start-0.25 + 4, y_start + 0.9), text="Stat. error", verticalalignment="center", zorder=9, fontsize=font_size_tot_eff)
...
plt.savefig("plots/ALICE-ITS3_"+formatted_date+"_APTS-SF_inpix_eff_"+CHIP+"_"+str(int(thr))+"e.pdf")
plt.savefig("plots/ALICE-ITS3_"+formatted_date+"_APTS-SF_inpix_eff_"+CHIP+"_"+str(int(thr))+"e.png")
# plt.show()
