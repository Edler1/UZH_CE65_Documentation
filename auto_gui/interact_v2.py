...
def start_gui_acquisition(chip="C4", VSUB=0, HV=10, temp=20):
    if(chip=="A4" or chip=="B4" or chip=="C4" or chip=="D4"):
        raise NotImplementedError("Please only use this script for V2. Chip selection is hardcoded for now...")
    ...
    ce65_directory = "/home/fcc-ce65-testsetup/CE65/ce65_daq_v2"
    ...
    ## Sadly for now v2 requires chip variant D4, I look forward to this changing to something more appropriate...
    pyautogui.click(*tuple(coords["chip_variant_D4"]))
    time.sleep(1)
    pyautogui.click(*tuple(coords["proxy_board"]))
    time.sleep(1)
    pyautogui.click(*tuple(coords["proxy_board_006"]))
    time.sleep(1)
    ## Below is not implemented explicity in v2 yet...
